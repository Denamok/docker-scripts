...
then
 docker run -d --restart=always --name $consul_swarm_agent_container_name -e "SWARM_HOST=:2375" swarm join --advertise "$(docker-machine ip $consul_hostname):2376" consul://$(docker-machine ip $consul_hostname):8500
 docker network create -d overlay lan
else
...
then
docker run --restart=always --name ${registrator_master_container_name} -v /var/run/docker.sock:/tmp/docker.sock -h ${registrator_master_hostname} -e constraint:node==$swarm_master_hostname --net=lan vp-aix-docker-registry.aix.vpg.lan:5000/vp-registrator:experimental -internal consul://$(docker-machine ip $consul_hostname):8500
else
...
do
  ...
  then
    docker run -d --name ${registrator_agent_container_name} -e constraint:node==${swarm_agent_hostname}${i} --net=lan --volume=/var/run/docker.sock:/tmp/docker.sock gliderlabs/registrator:latest consul://$(docker-machine ip $consul_hostname):8500
  else
